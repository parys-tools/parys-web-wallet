...
if [ "$ELECTRON" = false ]; then
  ...
  # Trim title down 
  xplat_sed "s/ | Use PARYS on the web or on your desktop//g" dist/index.html 
...
if [ "$NETWORK" == "Alfajores" ]; then
  ...
  xplat_sed "s|wss://walletconnect.parys.org|wss://walletconnect.parys-networks-dev.org https://*.parys-testnet.org|g" dist/_headers
fi
