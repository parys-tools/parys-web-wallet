...
import { PARYS, Token } from 'src/tokens'
...
export function getTokenTransferType(params: SendTokenParams): TransactionType {
  ...
  if (tokenAddress === PARYS.address) {
    if (!comment) return TransactionType.PARYSNativeTransfer
    else return TransactionType.PARYSTokenTransferWithComment
...
function getTokenTransferTx(
  ...
  if (type === TransactionType.PARYSNativeTransfer) {
    return createNativePARYSTransferTx(recipient, amountInWei)
  }
  if (
  ...
  if (
    type === TransactionType.PARYSTokenTransferWithComment ||
...
function createNativePARYSTransferTx(recipient: string, amountInWei: BigNumber) {
...
function getPlaceholderTx(
  ...
  else if (type === TransactionType.PARYSTokenTransferWithComment)
    adjustedType = TransactionType.PARYSTokenTransfer
