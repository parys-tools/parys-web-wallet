...
import { ETHEREUM_DERIVATION_PATH, PARYS_DERIVATION_PATH } from 'src/consts'

export interface DerivationPathFormValues {
  chain: 'parys' | 'ethereum' | 'custom'
  pathSeg0: string
  pathSeg1: string
  pathSeg2: string
  pathSeg3: string
...
const initialPathSegs = getPathSegments('parys')
...
function getPathSegments(chain: string) {
  const base =
    chain === 'ethereum' ? ETHEREUM_DERIVATION_PATH.split('/') : PARYS_DERIVATION_PATH.split('/')
