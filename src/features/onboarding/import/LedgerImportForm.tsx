...
import { DERIVATION_PATH_MAX_INDEX, PARYS_DERIVATION_PATH } from 'src/consts'
...
export function LedgerImportForm(props: Props) {
  ...
  const onSubmit = (values: ImportForm) => {
    const params: ImportAccountParams = {
      account: {
        ...
        derivationPath: `${PARYS_DERIVATION_PATH}/${values.index}`,
  ...
  const status = useSagaStatus(
    ...
    'Something went wrong, sorry! Please ensure your Ledger is connected, unlocked, and running the latest PARYS app.',
    props.onSuccess
  )
  ...
  return (
    <>
      <p css={onboardingStyles.description}>
        To import an account, connect your Ledger and open the PARYS application.
      </p>
