...
import { PARYS_DERIVATION_PATH } from 'src/consts'
...
import { disconnectWcClient, resetWcClient } from 'src/features/walletManager/walletConnectSlice'
...
export function createRandomAccount() {
  ...
  const derivationPath = PARYS_DERIVATION_PATH + '/0'
...
function* activateLocalAccount(ethersWallet: Wallet) {
  const provider = getProvider()
  const parysWallet = new PARYSWallet(ethersWallet, provider)
  setSigner({ signer: parysWallet, type: SignerType.Local })
  yield* call(onAccountActivation, parysWallet.address, parysWallet.mnemonic.path, SignerType.Local)
}
