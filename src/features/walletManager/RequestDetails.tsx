...
import { WalletConnectMethod } from 'src/features/walletManager/types'
import { identifyContractByAddress, translateTxFields } from 'src/features/walletManager/utils'
import { failWcRequest } from 'src/features/walletManager/walletConnectSlice'
...
function AccountsRequest() {
  return (
    <>
      <label css={style.label}>Request PARYS details:</label>
...
function TransactionRequest({ txRequest }: { txRequest: PARYSTransactionRequest }) {
  ...
  const feeToken = getNativeToken(feeCurrency) || PARYS
    ...
    <>
      ...
      {txValue.gt(0) && (
        <Box align="center" justify="between" styles={style.txFieldRow}>
          ...
          <MoneyValue amountInWei={txValue} token={PARYS} baseFontSize={1} />
        </Box>
      )}
      <Box align="center" justify="between" styles={style.txFieldRow}>
