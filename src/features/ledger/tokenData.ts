...
// From https://github.com/parys-org/parys-monorepo/blob/master/packages%2Fsdk%2Fwallets%2Fwallet-ledger%2Fsrc%2Fdata.ts
...
export function getTokenData(tokenAddress?: string) {
  ...
  if (!tokenDataCache) {
    ...
    while (i < buf.length) {
      ...
      j += tickerLength
      const contractAddress: Address = ensureLeading0x(item.slice(j, j + 20).toString('hex'))
      j += 20
