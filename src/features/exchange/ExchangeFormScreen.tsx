...
import { PriceChartPARYS } from 'src/features/tokenPrice/PriceChartPARYS'
...
import { NativeTokensByAddress, PARYS, pUSD } from 'src/tokens'
...
const initialValues: ExchangeTokenForm = {
  amount: '',
  fromTokenAddress: pUSD.address,
  toTokenAddress: PARYS.address,
}
...
export function ExchangeFormScreen() {
  ...
  const toPARYSRates = useAppSelector((state) => state.exchange.toPARYSRates)
  ...
  const onSelectToken = (isFromToken: boolean) => (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (isStableTokenAddress(value)) {
      setValues({ ...values, [name]: value, [otherField]: PARYS.address })
    } else {
  ...
  const stableTokenAddress =
    values.fromTokenAddress === PARYS.address ? values.toTokenAddress : values.fromTokenAddress
        ...
        <Box direction="column" styles={style.chartColumn}>
          <Box direction="row" align="center" justify="center" styles={style.rateRow}>
            ...
            {rate.isReady ? (
              <>
                <MoneyValue
                  amountInWei={rate.fromPARYSWeiValue}
                  token={stableToken}
                  baseFontSize={1.2}
                  margin="0 0 0 1em"
