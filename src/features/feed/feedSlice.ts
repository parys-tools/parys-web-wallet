...
import { PARYSTransaction, TransactionMap, TransactionType } from 'src/features/types'
...
const feedSlice = createSlice({
  ...
  reducers: {
    ...
    addPlaceholderTransaction: (state, action: PayloadAction<PARYSTransaction>) => {
...
function mergeTransactions(oldTxs: TransactionMap, newTxs: TransactionMap) {
  ...
  for (const tx of Object.values(newTxs)) {
    if (tx.type === TransactionType.ValidatorActivatePARYS && oldTxs[tx.hash]) {
