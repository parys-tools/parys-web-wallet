...
import { config, PARYSContract } from 'src/config'
...
export function TransactionStatusProperty({ tx }: { tx: PARYSTransaction }) {
  ...
  return (
    <TransactionProperty label="Status">
      ...
      <div css={[txReviewStyles.value, txReviewStyles.link]}>
        <TextLink link={`${config.blockscoutUrl}/tx/${tx.hash}`}>View on PARYS Explorer</TextLink>
      </div>
    </TransactionProperty>
  )
}
...
export function TransactionAmountProperty({ tx }: { tx: PARYSTransaction }) {
  ...
  return (
    <TransactionProperty label="Amount">
      <Box styles={txReviewStyles.value}>
        ...
        <MoneyValue amountInWei={tx.value} token={PARYS} />
      </Box>
      <Box styles={txReviewStyles.value}>
...
export function TransactionFeeProperty({ tx }: { tx: PARYSTransaction }) {
  const { feeValue, feeCurrency } = getFeeFromConfirmedTx(tx)
  return (
...
export function TransactionToAddressProperty({ tx }: { tx: PARYSTransaction }) {
...
export function TransactionContractProperty({ tx }: { tx: PARYSTransaction }) {
...
function getContractDetails(tx: PARYSTransaction) {
  const details: {
    name: PARYSContract | null
    method: string | null
  } = {
    name: null,
    method: null,
