...
import { NativeTokensByAddress, PARYS, Token } from 'src/tokens'
import { areAddressesEqual, isValidAddress, normalizeAddress } from 'src/utils/addresses'
import { logger } from 'src/utils/logger'

const SENTINEL_INVITE_COMMENT = '__PARYS_INVITE_TX__'
...
export function parseTransaction(
  ...
  if (exchangesByAddress[to]) {
    ...
    return parseExchangeTx(
      ...
      abiInterfaces[PARYSContract.Exchange]!
    )
  }

  if (areAddressesEqual(to, config.contractAddresses[PARYSContract.GoldToken])) {
    return parseOutgoingTokenTx(tx, PARYS, abiInterfaces[PARYSContract.GoldToken]!)
  }

  if (tokensByAddress[to]) {
    // If recipient was a known token
    // Note, using StableToken ABI as its a superset of ERC20 ABI
    return parseOutgoingTokenTx(tx, tokensByAddress[to], abiInterfaces[PARYSContract.StableToken]!)
  }

  if (areAddressesEqual(to, config.contractAddresses[PARYSContract.Escrow])) {
    return parseOutgoingEscrowTx(tx, address, tokensByAddress, abiInterfaces)
  }

  if (areAddressesEqual(from, config.contractAddresses[PARYSContract.Escrow])) {
    // TODO parse escrow incoming
  }

  if (areAddressesEqual(to, config.contractAddresses[PARYSContract.LockedGold])) {
    return parseLockedGoldTx(tx, abiInterfaces)
  }

  if (areAddressesEqual(to, config.contractAddresses[PARYSContract.Election])) {
    return parseElectionTx(tx, abiInterfaces)
  }

  if (areAddressesEqual(to, config.contractAddresses[PARYSContract.Governance])) {
...
function parseTokenExchange(
  ...
  return {
    ...
    fromTokenId: sellGold ? PARYS.address : token.address,
    toTokenId: sellGold ? token.address : PARYS.address,
...
function parseOutgoingTokenTransfer(
...
): StableTokenTransferTx | PARYSTokenTransferTx | OtherTokenTransferTx {
  ...
  if (token.address === PARYS.address) {
    return { ...result, type: TransactionType.PARYSTokenTransfer, tokenId: token.address }
...
function parseTokenApproveTx(
...
): StableTokenApproveTx | PARYSTokenApproveTx | OtherTokenApproveTx {
  ...
  if (token.address === PARYS.address) {
    return { ...result, type: TransactionType.PARYSTokenApprove, tokenId: token.address }
  } else if (isStableToken(token)) {
...
function parseTxWithTokenTransfers(
  ...
  address: Address,
  abiInterfaces: AbiInterfaceMap
): StableTokenTransferTx | PARYSTokenTransferTx | OtherTokenTransferTx | OtherTx | null {
  ...
  try {
    ...
    if (tokenAddress === PARYS.address) {
      return { ...result, type: TransactionType.PARYSTokenTransfer, tokenId: tokenAddress }
...
function parseOutgoingEscrowTx(
  ...
  try {
    const abiInterface = abiInterfaces[PARYSContract.Escrow]!
    const txDescription = abiInterface.parseTransaction({ data: tx.input, value: tx.value })
    const name = txDescription.name
...
function parseLockedGoldTx(
  ...
  try {
    const abiInterface = abiInterfaces[PARYSContract.LockedGold]!
    const txDescription = abiInterface.parseTransaction({ data: tx.input, value: tx.value })
    const name = txDescription.name
    ...
    if (name === 'lock') {
      return {
        ...parseOtherTx(tx),
        type: TransactionType.LockPARYS,
      }
    }
    if (name === 'relock') {
      return {
    ...
    if (name === 'unlock') {
      return {
        ...parseOtherTx(tx),
        type: TransactionType.UnlockPARYS,
        value: BigNumber.from(txDescription.args.value).toString(),
      }
    }
    if (name === 'withdraw') {
...
function parseElectionTx(tx: BlockscoutTx, abiInterfaces: AbiInterfaceMap): StakeTokenTx | OtherTx {
  try {
    const abiInterface = abiInterfaces[PARYSContract.Election]!
    const txDescription = abiInterface.parseTransaction({ data: tx.input, value: tx.value })
    const name = txDescription.name
    ...
    if (name === 'activate') {
      return {
        ...parseOtherTx(tx),
        type: TransactionType.ValidatorActivatePARYS,
        groupAddress,
      }
    }
...
function parseGovernanceTx(
  ...
  try {
    const abiInterface = abiInterfaces[PARYSContract.Governance]!
    const txDescription = abiInterface.parseTransaction({ data: tx.input, value: tx.value })
    const name = txDescription.name
...
function tryParseTransferComment(
  ...
  try {
    ...
    const abiInterface =
      tokenAddress === PARYS.address
        ? abiInterfaces[PARYSContract.GoldToken]
        : abiInterfaces[PARYSContract.StableToken]
    const txDescription = abiInterface!.parseTransaction({ data: tx.input })
...
function parseNativeTransferTx(tx: BlockscoutTx, address: Address): PARYSNativeTransferTx {
  return {
    ...parseOtherTx(tx),
    type: TransactionType.PARYSNativeTransfer,
    isOutgoing: areAddressesEqual(tx.from, address),
    comment: undefined,
    tokenId: PARYS.address,
  }
}
...
function tokenSymbolToAddress(
  ...
  if (!symbol || symbol.toLowerCase() === 'cgld') return PARYS.address
