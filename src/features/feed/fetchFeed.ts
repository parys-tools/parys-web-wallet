...
function getAbiInterfacesForParsing(): AbiInterfaceMap {
  const goldTokenContract = getContract(PARYSContract.GoldToken)
  const stableTokenContract = getContract(PARYSContract.StableToken)
  const exchangeContract = getContract(PARYSContract.Exchange)
  const escrowContract = getContract(PARYSContract.Escrow)
  const lockedGoldContract = getContract(PARYSContract.LockedGold)
  const electionContract = getContract(PARYSContract.Election)
  const governanceContract = getContract(PARYSContract.Governance)

  return {
    [PARYSContract.GoldToken]: goldTokenContract.interface,
    [PARYSContract.StableToken]: stableTokenContract.interface,
    [PARYSContract.Exchange]: exchangeContract.interface,
    [PARYSContract.Escrow]: escrowContract.interface,
    [PARYSContract.LockedGold]: lockedGoldContract.interface,
    [PARYSContract.Election]: electionContract.interface,
    [PARYSContract.Governance]: governanceContract.interface,
  }
}
