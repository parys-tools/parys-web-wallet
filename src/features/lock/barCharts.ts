...
import { getTotalPARYS, getTotalPendingPARYS, hasPendingPARYS } from 'src/features/lock/utils'
...
export function getSummaryChartData(balances: Balances, pendingWithdrawals: PendingWithdrawal[]) {
  const total = getTotalPARYS(balances)

  const unlocked = {
    label: 'Unlocked',
    value: fromWei(getTokenBalance(balances, PARYS)),
    color: Color.primaryGold,
  }
  const locked = {
    label: 'Locked',
    value: fromWei(balances.lockedPARYS.locked),
    color: Color.altGrey,
    labelColor: Color.chartGrey,
  }

  const pending = []
  if (hasPendingPARYS(balances)) {
    const pendingFree = BigNumber.from(balances.lockedPARYS.pendingFree)
    if (pendingFree.gt(0)) {
      pending.push({
...
export function getResultChartData(balances: Balances, values: LockTokenParams) {
  let unlocked = BigNumber.from(getTokenBalance(balances, PARYS))
  let pending = getTotalPendingPARYS(balances)
  let locked = BigNumber.from(balances.lockedPARYS.locked)
  const total = getTotalPARYS(balances)
