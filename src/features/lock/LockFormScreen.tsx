...
import { getTotalUnlockedPARYS } from 'src/features/lock/utils'
...
export function LockFormScreen() {
  ...
  // Show modal to recommend nav to stake form when unlocking staked PARYS
  const helpText = `You can't unlock PARYS that's currently staked with validators. You must revoke votes first. Would you like to change your votes now?`
  useNavHintModal(errors.stakedPARYS, 'Funds Are Still Voting', helpText, 'Change Votes', '/stake')

  const onSelectAction = (event: ChangeEvent<HTMLInputElement>) => {
    const { name, value } = event.target
    let autoSetAmount: string
    if (value === LockActionType.Withdraw) {
      autoSetAmount = fromWeiRounded(balances.lockedPARYS.pendingFree)
    } else {
      autoSetAmount = '0'
    }
  ...
  const onUseMax = () => {
    const { locked, pendingFree } = balances.lockedPARYS
    let maxAmount = '0'
    if (values.action === LockActionType.Lock) {
      maxAmount = fromWeiRounded(getTotalUnlockedPARYS(balances))
  ...
  return (
    <ScreenContentFrame>
      <h1 css={Font.h2Green}>
        Lock or Unlock PARYS <HelpButton />
      </h1>
