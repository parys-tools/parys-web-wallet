...
function balance(amount: BigNumberish, token = PARYS): Balances {
  return {
...
describe('validateAmount', () => {
  it('Validates min', () => {
    expect(validateAmount(-1, PARYS)).toBeTruthy()
    expect(validateAmount(0, PARYS)).toBeTruthy()
    expect(validateAmount(10, PARYS, null, null, 20)).toBeTruthy()
    expect(validateAmount(30, PARYS, null, null, 20)).toBeFalsy()
  })
  it('Validates max', () => {
    expect(validateAmount('1000000000000000000', PARYS, null, '500000000000000000')).toBeTruthy()
    expect(validateAmount('400000000000000000', PARYS, null, '500000000000000000')).toBeFalsy()
    // Amounts nearly equal allowed
    expect(validateAmount('500000000000000001', PARYS, null, '500000000000000000')).toBeFalsy()
  })
  it('Validates balances', () => {
    expect(validateAmount('1000000000000000000', PARYS, balance('500000000000000000'))).toBeTruthy()
    expect(validateAmount('400000000000000000', PARYS, balance('500000000000000000'))).toBeFalsy()
    // Amounts nearly equal allowed
    expect(validateAmount('500000000000000001', PARYS, balance('500000000000000000'))).toBeFalsy()
  })
})

describe('validateAmountWithFees', () => {
  it('Requires fee', () => {
    expect(validateAmountWithFees(30, PARYS, balance(30), undefined)).toBeTruthy()
  })
  it('Validates same fee currency', () => {
    expect(validateAmountWithFees(toWei(1), PARYS, balance(toWei(2)), fees)).toBeFalsy()
    expect(validateAmountWithFees(toWei(1), PARYS, balance(toWei(1)), fees)).toBeTruthy()
    expect(validateAmountWithFees(toWei(1), PARYS, balance(toWei(1.2)), fees)).toBeTruthy()
    expect(validateAmountWithFees(toWei(1), PARYS, balance(toWei(1.3)), fees)).toBeFalsy()
  })
  it('Validates diff fee currency', () => {
    expect(validateAmountWithFees(toWei(1), pUSD, balance(toWei(2)), fees)).toBeFalsy()
    expect(validateAmountWithFees(toWei(1), pUSD, balance(toWei(0.1)), fees)).toBeTruthy()
  })
})

describe('getAdjustedAmountFromBalances', () => {
  it('Adjusts price correctly', () => {
    expect(getAdjustedAmountFromBalances(toWei(1), pUSD, balance(toWei(2)), fees)).toEqual(toWei(1))
    expect(getAdjustedAmountFromBalances(toWei(1), pUSD, balance(toWei(1)), fees)).toEqual(toWei(1))
    expect(getAdjustedAmountFromBalances(toWei(1), PARYS, balance(toWei(2)), fees)).toEqual(
      toWei(1)
    )
    expect(getAdjustedAmountFromBalances(toWei(1), PARYS, balance(toWei(1)), fees)).toEqual(
      toWei(0.7)
    )
  })
})
...
describe('getAdjustedAmount', () => {
  it('Adjusts price correctly', () => {
    expect(getAdjustedAmount(toWei(1), toWei(2), PARYS)).toEqual(toWei(1))
    expect(getAdjustedAmount(toWei(1), toWei(1.0001), PARYS)).toEqual(toWei(1.0001))
  })
})

describe('areAmountsNearlyEqual', () => {
  it('Checks standard tokens', () => {
    expect(areAmountsNearlyEqual(toWei(1), toWei(2), PARYS)).toBeFalse()
    expect(areAmountsNearlyEqual(toWei(1), toWei(1), PARYS)).toBeTrue()
    expect(areAmountsNearlyEqual(toWei(1), toWei(1 + MIN_DISPLAY_VALUE * 0.9), PARYS)).toBeTrue()
    expect(areAmountsNearlyEqual(toWei(1), toWei(1 + MIN_DISPLAY_VALUE), PARYS)).toBeFalse()
  })
